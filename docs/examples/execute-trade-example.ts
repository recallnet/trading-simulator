  ...
  try {
    ...
    */
    ...
    if (result && result.success && result.transaction) {
      ...
      console.log(
        `Sold: ${result.transaction.fromAmount} tokens at address ${result.transaction.fromToken}`,
      );
      console.log(
        `Received: ${result.transaction.toAmount} tokens at address ${result.transaction.toToken}`,
      );
      console.log(`Execution price: ${result.transaction.price}`);
      console.log(`Timestamp: ${result.transaction.timestamp}`);
      console.log(
        `From chain: ${result.transaction.fromChain} (${result.transaction.fromSpecificChain || 'unspecified'})`,
      );
      console.log(
        `To chain: ${result.transaction.toChain} (${result.transaction.toSpecificChain || 'unspecified'})`,
      );
    }

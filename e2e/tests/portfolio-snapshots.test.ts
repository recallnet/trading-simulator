...
describe('Portfolio Snapshots', () => {
  ...
  test('manually taking snapshots creates new portfolio snapshots', async () => {
    ...
    const initialSnapshotsResponse = await adminClient.request('get', `/api/admin/competition/${competitionId}/snapshots`) as SnapshotResponse;
    ...
    // Get snapshots again
    const afterFirstSnapshotResponse = await adminClient.request('get', `/api/admin/competition/${competitionId}/snapshots`) as SnapshotResponse;
    ...
    // Get snapshots again
    const afterSecondSnapshotResponse = await adminClient.request('get', `/api/admin/competition/${competitionId}/snapshots`) as SnapshotResponse;
  ...
  test('takes a snapshot when a competition ends', async () => {
    ...
    const beforeEndResponse = await adminClient.request('get', `/api/admin/competition/${competitionId}/snapshots`) as SnapshotResponse;
    ...
    const afterEndResponse = await adminClient.request('get', `/api/admin/competition/${competitionId}/snapshots`) as SnapshotResponse;
  ...
  test('calculates portfolio value correctly based on token prices', async () => {
    ...
    const usdcTokenAddress = config.specificChainTokens.svm.usdc;
    const initialUsdcBalance = initialBalanceResponse.balances.find(b => b.token === usdcTokenAddress)?.amount || 0;
  ...
  test('reuses prices within freshness threshold', async () => {
    ...
    try {
      ...
      const priceReuseMessages = logMessages.filter(msg => 
      ...
      const dbHitMessages = logMessages.filter(msg =>
        msg.includes('[CompetitionManager]') && 
      ...
      if (priceReuseMessages.length === 0) {
        ...
        const specifcChainMessages = logMessages.filter(msg => 
          msg.includes('[CompetitionManager]') && 
          msg.includes('Using specific chain info from DB'));
        
        if (specifcChainMessages.length > 0) {
          console.log(`[Test] Found ${specifcChainMessages.length} messages about using chain info from DB but not price`);

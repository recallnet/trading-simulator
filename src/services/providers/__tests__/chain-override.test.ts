  ...
  describe('Direct provider tests with chain override', () => {
    it('should successfully fetch prices when providing the exact chain for at least one token', async () => {
      ...
      for (const token of testTokens) {
        console.log(`Testing ${token.name} (${token.address}) with specified chain: ${token.expectedChain}`);
        ...
        const price = await multiChainProvider.getPrice(token.address, BlockchainType.EVM, token.expectedChain);
        ...
        if (price !== null) {
          ...
          console.log(`Could not get price for ${token.name} on ${token.expectedChain} - this might be due to API timeouts or rate limiting`);
        }
      }
    ...
    it('should be faster to get prices with chain override than without', async () => {
      ...
      for (const token of testTokens) {
        ...
        console.log(`Difference: ${timeWithout - timeWith}ms (${((timeWithout - timeWith) / timeWithout * 100).toFixed(2)}% faster)`);

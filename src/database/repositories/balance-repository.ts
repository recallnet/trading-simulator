...
export class BalanceRepository extends BaseRepository<Balance> {
   ...
   */
  async saveBalance(teamId: string, tokenAddress: string, amount: number, client?: PoolClient): Promise<Balance> {
    try {
      const query = `
   ...
   * @param client Optional database client for transactions
   */
  async getBalance(teamId: string, tokenAddress: string, client?: PoolClient): Promise<Balance | null> {
    try {
      const query = `
   ...
   * @param client Optional database client for transactions
   */
  async initializeTeamBalances(teamId: string, initialBalances: Map<string, number>, client?: PoolClient): Promise<void> {
    try {
   ...
   * @param client Optional database client for transactions
   */
  async resetTeamBalances(teamId: string, initialBalances: Map<string, number>, client?: PoolClient): Promise<void> {
    try {
      // Use a transaction if no client is provided
      if (!client) {
        await this.db.transaction(async (transactionClient) => {

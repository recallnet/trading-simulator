...
export class PriceRepository extends BaseRepository<PriceRecord> {
  ...
  async create(priceData: PriceRecord): Promise<PriceRecord> {
    ...
    try {
      ...
      if (specificChainColumnExists && priceData.specificChain) {
        ...
        values = [priceData.token, priceData.price, priceData.timestamp, priceData.chain];
      } else {
        ...
        values = [priceData.token, priceData.price, priceData.timestamp];
      }
  ...
  protected mapToEntity(data: DatabaseRow): PriceRecord {
    return {
      ...
      chain: data.chain as BlockchainType | undefined,
      specificChain: (data.specificChain as SpecificChain | undefined) || undefined,
    };
  }
}

...
const checkDatabase = async () => {
  ...
  try {
    ...
    if (missingTables === 0) {
      ...
      console.log(`${colors.red}✗ No required tables found - database schema is not initialized${colors.reset}`);
      console.log(`${colors.yellow}Run 'npm run db:init' to initialize the database schema${colors.reset}`);
    } else {
      console.log(`${colors.yellow}⚠ Some tables are missing - database schema is incomplete${colors.reset}`);
      console.log(`${colors.yellow}Run 'npm run db:reset' to reset and fully initialize the database${colors.reset}`);
    }

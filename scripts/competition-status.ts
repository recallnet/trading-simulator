...
async function showCompetitionStatus() {
  try {
    ...
    console.log(`${colors.cyan}║                   COMPETITION STATUS                          ║${colors.reset}`);
    console.log(`${colors.cyan}╚════════════════════════════════════════════════════════════════╝${colors.reset}`);
    ...
    if (!competition) {
      ...
      if (completedCompetitions.length > 0) {
        ...
        completedCompetitions.forEach((comp: any, index: number) => {
          ...
          console.log(`${index + 1}. ${comp.name} (${formatDate(startDateValue)} - ${formatDate(endDateValue)})`);
        });
    ...
    const participatingTeamIds = await repositories.competitionRepository.getCompetitionTeams(competition.id);
    ...
    console.log(`${colors.green}Description:${colors.reset} ${competition.description || '(none)'}`);
    ...
    console.log(`${colors.green}Duration:${colors.reset} ${calculateDuration(competition.startDate)}`);
    ...
    console.error(`\n${colors.red}Error fetching competition status:${colors.reset}`, error instanceof Error ? error.message : error);
  } finally {

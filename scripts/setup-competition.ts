...
async function selectTeams(): Promise<Team[]> {
  try {
    ...
    teams.forEach((team, index) => {
      console.log(`${colors.green}${index + 1}.${colors.reset} ${colors.white}${team.name}${colors.reset} ${colors.gray}(${team.id})${colors.reset}`);
    ...
    console.error(`\n${colors.red}Error selecting teams:${colors.reset}`, error instanceof Error ? error.message : error);
    return [];
  }
}
...
async function setupCompetition() {
  try {
    console.log(`${colors.cyan}╔════════════════════════════════════════════════════════════════╗${colors.reset}`);
    console.log(`${colors.cyan}║                     COMPETITION SETUP                         ║${colors.reset}`);
    console.log(`${colors.cyan}╚════════════════════════════════════════════════════════════════╝${colors.reset}`);
    ...
    const description = await prompt(`${colors.yellow}Enter competition description (optional):${colors.reset} `);
    ...
    console.log(`${colors.blue}Starting competition with ${teamIds.length} teams...${colors.reset}`);
    ...
    console.error(`\n${colors.red}Error setting up competition:${colors.reset}`, error instanceof Error ? error.message : error);
  } finally {
    rl.close();
